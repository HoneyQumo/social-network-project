...
const LoginForm = ({ isAuth, login }) => {
	...
	const validationSchema = Yup.object().shape({
		email: Yup.string().email('Почта указана неправильно').required('Введите почту'),
			...
			>
				{({ values, errors, touched, handleChange, handleBlur, isValid, handleSubmit, dirty, status }) => (
					<div className='login__form'>
						...
						<button className='login__button' disabled={!isValid && !dirty} onClick={handleSubmit} type='submit'>
							Войти
						</button>
					</div>
				)}
